...
func TestParseListFixture(t *testing.T) {
	...
	doc, err := crawler.ParseHTML(body, "https://www.jackroad.co.jp/shop/r/rjwrx/")
	if err != nil {
		t.Fatal(err)
	}
	items := ParseList(doc)
	...
	withPrice, withBrand, withRef, withImg := 0, 0, 0, 0
	for _, l := range items {
	...
	t.Logf("items=%d price=%d brand=%d ref=%d img=%d sample=%+v", len(items), withPrice, withBrand, withRef, withImg, items[0])
